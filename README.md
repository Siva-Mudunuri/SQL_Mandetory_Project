...
WHERE booking.date_to IS NOT NULL;
...
SELECT (COUNT(DISTINCT booking.room_no) / COUNT(room.room_no)) * 100 as occupancy_rate
FROM booking
JOIN room ON booking.room_no = room.room_no
JOIN hotel ON room.hotel_no = hotel.hotel_no
WHERE booking.date_from <= CURRENT_DATE AND (booking.date_to IS NULL OR booking.date_to >= CURRENT_DATE);
